...
class Log(QtGui.QDockWidget):
    """
        Log Dock Widget for use in MyGui.MainGui        
        Simply continue using e.g. logging.info() ...
        
        usage:
            log_dock = MyGui.Log.Log()
    ...
    def __init__(self,datefmt=None,infoString = True):
        ...
        # remove eventually existing log file (otherwise it appends to it)
        try:
...
def run():
    ...
    win.addDockWidget(QtCore.Qt.TopDockWidgetArea, Log())
    win.show()
    sys.exit(app.exec_())
